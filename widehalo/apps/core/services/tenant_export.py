...
def import_tenant_archive(archive_bytes: bytes, *, target_tenant: Tenant) -> dict[str, int]:
    ...
    objet importe, et on reporte ce remappage sur les references generiques
    internes a l'archive (`content_type`/`object_id`) pour ne pas casser les
    liens entre objets exportes ensemble."""
    ...
    with activate_tenant(target_tenant.id):
        for label, objects in all_objects.items():
            for deserialized_obj in objects:
                imported: Any = deserialized_obj.object
