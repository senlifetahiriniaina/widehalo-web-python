...
def apply_adjustment(
    ...
    before = (
        ...
        else forecast.statistical_value
    )
...
def revert_adjustment(forecast: ForSeriesForecast, *, user: User, reason: str) -> ForSeriesForecast:
    ...
    return apply_adjustment(
        forecast,
        new_value=forecast.statistical_value,
        reason=reason or "Retour à la prévision statistique",
        user=user,
    )


def measure_adjustment_contribution(
    forecast: ForSeriesForecast, *, actual_value: Decimal
) -> ForSeriesForecast:
    ...
    if actual_value == 0:
        return forecast
    forecast.statistical_error_pct = (
        abs(actual_value - forecast.statistical_value) / abs(actual_value) * 100
    )
    if forecast.adjusted_value is not None:
        forecast.adjustment_error_pct = (
            abs(actual_value - forecast.adjusted_value) / abs(actual_value) * 100
        )
