...
| **L1 bis** — second relèvement, seuil de la Vague 2 | ✅ livré | Celui que le premier annonçait, au moment prévu : Vague 1 close, Vague 2 pas commencée. **+25 % décidé par l'utilisateur** — 519 / 1 000 / 400. Consommation mesurée au relèvement : 305 / 589 / 252. À 1 000, les endpoints restaient **sous** la projection Phase 4 (1 210), l'écart venant presque entièrement de la surface d'API publique du **Bloc B** (S7–S9) : le plafond serait redevenu bloquant **en cours** de Vague 2, à ce bloc précis. Réserve levée dans la foulée, **sur décision de l'utilisateur** : **+500 endpoints, 1 000 → 1 500**. Les trois plafonds couvrent désormais la Phase 4 (519 > 430, 1 500 > 1 210, 400 > 278) et aucun troisième arbitrage n'est à prévoir en cours de vague. Restent à poser les **deux budgets nouveaux** que le cahier annonce (12 adaptateurs, 80 opérations publiques) : ce sont eux qui borneront la surface d'API là où un compteur global d'endpoints ne le fait pas. |
...
- **Tout champ `type="number"` prérempli perdait sa valeur.** Django rend un `Decimal`
  avec une virgule sous locale française ; le navigateur juge cette valeur invalide et
  la lit comme une chaîne vide. Sept champs touchés — le kanban atelier enregistrait
  0 pièce produite, la fiche partenaire effaçait l'encours autorisé, la configuration
  WhatsApp effaçait le plafond de coût. Trouvé par le test e2e de **L14** écrit pour
  tout autre chose, corrigé par `|unlocalize` et fermé par une garde.
...
| **L13** — inventaire à l'aveugle | ✅ livré | Le gabarit masquait déjà, et son propre commentaire prévoyait le risque mot pour mot (« si une future API sérialise StkInventoryLine, elle doit reprendre la même garde ») : elle ne l'avait pas repris. **Deux fuites** fermées — `POST .../lines` et la feuille STK-INV, dont le lien de téléchargement est affiché juste sous le tableau masqué. Le mode devient explicite (`is_blind`) là où le masquage était inconditionnel et non déclaré. |
...
| **L14** — paie et kanban de production | ✅ livré | La régularisation était bien une recopie intégrale, comme le plan le supposait — mais le plan sous-estimait la portée : la recopie ne faussait pas un montant, elle en faussait **quatre**. Brut, cotisations, IRSA et net repartaient à plein sur un bulletin rectificatif, et les retenues d'avance étaient réimputées une seconde fois sur le solde du salarié. Le kanban `mrp` en réseau dégradé, lui, était sain ; sa docstring niait pourtant le glisser-déposer que son propre gabarit implémente. |
| **Bloquants (1/3)** — la facture légale, le canal WhatsApp entrant | ✅ livré | SAL-8 : `invoice_pdf` n'était pas une facture (cf. audit §6). WA-1/WA-2/WA-7 : tout message entrant était écrit sans tenant donc invisible, « STOP » ne désabonnait personne, et `WHATSAPP_DEFAULT_TENANT_ID` n'était semé dans aucun fichier de déploiement. |
| **Bloquants (2/3)** — le devis au clavier, l'écriture vide, les commentaires qui fuyaient | ✅ livré | SAL-1 : le parcours clavier n'était pas lent, il était **impossible**. ACC-3 : une écriture sans aucune ligne passait le contrôle d'équilibre et consommait un numéro de séquence légale. Plus 24 commentaires `{# … #}` multilignes rendus dans le HTML, dont l'un capturait le focus du parcours SAL-1 lui-même. |
| **Bloquants (3/3)** — la TVA de vente (rattachée à **L5** sur décision du maître d'ouvrage ; SAL-6 et SAL-7 restent ouverts) | ✅ livré | Le plan ne mentionnait pas la TVA côté `sales`, et l'audit la classait ✅ sous SAL-2/SAL-3. **Aucune facture émise depuis une commande de vente n'avait jamais crédité de compte de TVA collectée** : `amount_tax` était écrit en dur à zéro sur le devis comme sur la commande, `tax_id` n'était renseigné par aucune surface d'entrée, et `invoice_order` n'envoyait à `accounting` que du hors taxe. Rien ne rougissait, parce qu'aucun jeu de données du dépôt ne crée d'`AccTax` de vente sur ce chemin. Deux défauts trouvés en le branchant : `vat_applicable` déclarait assujetti le régime `reel_sans_tva`, et `get_default_sale_tax` — la surface réellement consommée — ne lisait jamais le régime fiscal. `Tenant.vat_opted_in` trouve au passage son premier lecteur. Deux conséquences signalées, non corrigées ici : les seuils d'approbation de facture passent du HT au TTC, et les ratios DSO/BFR rapportent désormais des créances TTC à un chiffre d'affaires HT. |

| **Bloquants (4/4)** — trois régressions atteignables | ✅ livré | Trois défauts que le produit portait *malgré* des correctifs déjà écrits ailleurs. (1) Le webhook WhatsApp **historique** restait monté et rétablissait intégralement le défaut « message entrant sans tenant, donc invisible » fermé quelques jours plus tôt — retiré, avec note de migration d'exploitation. (2) Le périmètre RG-CRM-5, tenu par la vue web et par `list_leads`, était absent de **quatre** endpoints d'API : un commercial pouvait déplacer, gagner ou perdre l'opportunité d'un collègue par simple connaissance de son UUID. (3) RG-ACC-5 n'était tenue qu'en lecture : l'écran de configuration créait une `AccTax` sur un tenant non assujetti sans rien dire. Le test de falsification a d'ailleurs révélé un **faux vert dans mes propres tests** — un `grant_role("admin")` déclenchait l'exigence MFA, les requêtes n'atteignaient jamais la vue, et le refus semblait fonctionner pour la mauvaise raison. |

| **L17** — le régime de l'Impôt Synthétique | ✅ livré | **Lot ouvert par une question du maître d'ouvrage**, et elle visait juste : la TVA livrée la veille était juste au calcul et fausse à l'affichage. Sept surfaces imprimaient « TVA : 0 » entre un « Total HT » et un « Total TTC » à un tenant qui ne collecte aucune TVA — dont la facture légale, archivée et immuable, et le ticket POS remis au client. Cause commune : aucune surface ne lisait `vat_applicable`, toutes conditionnaient sur un **montant** au lieu d'un **régime**. Quatre défauts trouvés en instruisant la question : deux paramètres réglementaires déclarés par le cahier et semés nulle part ; un régime fiscal éditable seulement par `/admin/` ; un bac à sable qui perdait le régime ; un calendrier fiscal qui promettait à un tenant synthétique une déclaration IRCM que le produit lui **refuse**. Plus : `sales` était le seul écrivain de `tax_lines`, donc `projects` et `logistics` facturaient sans TVA même un tenant assujetti ; et `simulation` **interdisait** à un tenant synthétique de construire un socle. Décision du maître d'ouvrage appliquée : mention automatique dérivée du régime, avec réserve OECFM — le dépôt ne contient aucun référentiel fiscal malgache. |

| **L4** — CRM | ✅ livré | Les cinq critères, dont quatre ❌. Le plan visait juste sur la matière disponible (`move_lead_to_stage` publiait déjà son événement, `mrp/kanban.html` était le patron complet, `create_partner`/`create_contact` existaient) : l'essentiel du lot est du **câblage**, pas de la logique nouvelle. Deux points méritent d'être retenus. (1) Le chatter sur `CrmLead` **exigeait** sa garde de périmètre : sans elle, le repli de `core.views.chatter` est la permission de modèle `crm.view_crmlead`, et le fil de discussion de l'opportunité d'un collègue redevenait lisible — la faille des bloquants (4/4), sur une autre surface. (2) L'encours n'était pas seulement absent de la fiche : il n'était **pas une fonction**, il vivait dans le corps de `confirm_order`. Un test de falsification a par ailleurs découvert un défaut sans rapport avec les critères : la recherche de la liste CRM levait `FieldError` — **500 dès la première frappe** — parce qu'une colonne cherchable pointait sur une clé étrangère. Une seule occurrence dans le dépôt, corrigée génériquement et fermée par une garde. Reste ouvert et dit : la moitié « import d'un fichier » de CRM-5, aucun import CRM n'existant. |
...
|---|---|---|
| **S1** — cadrage et modèle | ✅ livré | Conforme, plus une correction : les budgets avaient **déjà** été relevés au-delà des cibles du cahier (519 / 1 500 / 400 contre 430 / 1 210 / 278) lors des deux arbitrages de la Vague 1 ; S1 n'a eu qu'à poser les **deux budgets neufs** (12 adaptateurs, 80 opérations publiques), avec leurs tests d'amorçage — les deux compteurs valant zéro, un test qui ne compte rien serait vert quelle que soit sa capacité à compter. |
| **S2** — machine à états | ✅ livré | Conforme. Le point de contrôle du cahier est tenu et **vérifié** : `AnFactEchange` ne prend qu'une clé de dimension conforme existante. Le test de contrôle, dans sa première version, n'assertait que l'absence de dépendance croisée — il ne pouvait donc pas voir le défaut qu'il décrivait. Trois corrections au modèle de S1 y sont jointes, dont l'absence de `cost_ariary`, trouvée en relisant le cahier **attribut par attribut** contre le modèle livré. |
| **S3** — file, réessai, disjoncteur | ✅ livré | **Trois écarts avec ce plan, tous en faveur du cahier**, conformément à la décision du maître d'ouvrage (« on s'en tient au critère du cahier des charges »). |
| S3 — écart 1 : le grain du disjoncteur | — | Ce plan écrivait « par connecteur et par tenant », reprenant §11 et §14.1 du cahier. Mais le **critère** FLX-3 et le dictionnaire §13.2 écrivent « sur une liaison », et les deux grains diffèrent réellement : la contrainte d'unicité des liaisons porte sur `(tenant, connector, name)`, donc un tenant tient plusieurs liaisons sur un même connecteur. L'axe A5 tranche définitivement — il place le **seuil de disjoncteur** sur la liaison, et un disjoncteur plus grossier que son propre seuil serait inapplicable. Livré **par liaison**. |
| S3 — écart 2 : trois exigences omises | — | Le plan ne mentionnait ni le **délai maximal par appel**, ni le **parallélisme borné par adaptateur**, ni la **file de worker dédiée** — les trois nommées par le cahier (§7.6 et §11). Toutes trois livrées. La troisième supposait une prémisse fausse : « isolée des deux existantes », alors que ce dépôt n'a jamais eu qu'**une** file. |
| S3 — écart 3 : le point d'ancrage | — | Le plan disait « file persistante adossée à `core/tasks.py::enqueue` ». La file **est la base** : un échange en `en file` attend, et rien d'autre. C'est ce que le cahier tranche lui-même (§12 : le courtier de messages dédié est « écarté ; réexamen si la file de sortie dépasse durablement le millier d'échanges en attente »). Django-Q ne sert qu'à **déclencher** une passe de vidange. La conséquence pratique compte : une passe tuée, un worker redémarré ou un déploiement en cours de vidange ne perdent aucun échange. |
| S3 — arbitrage H26 | 🟡 instruit, non tranché | Ce n'est pas une décision d'ingénierie mais du commanditaire, comme BI-3. Ce qui relevait de moi est fait : l'état des lieux du compteur existant est établi sur pièces (voir l'audit, §6 bis), et le compteur transverse est conçu pour que **les deux issues restent possibles** — le coût porte une unité, jamais un montant seul. Quatre défauts du compteur WhatsApp actuel sont relevés au passage, dont un sur-comptage : `conversation_id` est écrit par les trois écrivains et **agrégé nulle part**. |
...
| S3 | File de sortie, réessai, disjoncteur | File persistante adossée à `core/tasks.py::enqueue` (jamais d'import direct de `django_q`, garde CI existante) ; réessai espacé ; ~~disjoncteur par connecteur et par tenant~~ → **disjoncteur PAR LIAISON** (voir §3 ter) — la brique la plus neuve du bloc, aucun précédent dans le dépôt ; incident unique par famille d'erreur, jamais un incident par tentative. Arbitrage H26 (unité de coût de la messagerie) | `apps/flows/services/queue.py`, `apps/flows/models.py::Incident` | FLX-3 |
