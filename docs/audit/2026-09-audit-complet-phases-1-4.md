...
| §3.2 Budgets d'architecture | ❓ non mesurables | Mesurés le jour même (300 / 576 / 240) et relevés de +33 % à **415 / 800 / 320**. Puis, au seuil de la Vague 2, second relèvement de **+25 %** décidé par l'utilisateur : 519 / 1 000 / 400, pour une consommation mesurée de 305 / 589 / 252. Les endpoints restaient alors sous la projection Phase 4 (1 000 < 1 210, écart imputable au Bloc B) ; réserve levée aussitôt sur décision de l'utilisateur — **+500 endpoints, soit 519 / 1 500 / 400**. Les trois plafonds couvrent la Phase 4, et les deux budgets nouveaux du cahier (12 adaptateurs, 80 opérations publiques) restent à poser. |
| §3.1 L'ordonnancement n'est câblé nulle part | ❌ — dix-neuf commandes périodiques, aucune planifiée | Fermé par **L0** : registre déclaratif par module, écriture des planifications au déploiement (`sync_scheduled_commands`), écran d'exploitation, et garde qui refuse toute commande ni déclarée ni motivée. Les cinq commandes non idempotentes l'ont été d'abord (L0-1), et l'isolation d'erreur par tenant généralisée aux dix-huit qui en manquaient (L0-2). Exécution réelle relevée dans `DEPLOYMENT_HETZNER.md` §9 bis.3. |
| §3.5 Les gardes CI sont plus étroites que ce que les cahiers exigent | 🟡 | Fermé par **L2** : quatre gardes nouvelles (copilote en lecture seule, SQL brut hors du chemin des rapports, jours fériés, taux de TVA) et la garde paie élargie aux montants et bornes de tranches, `management/` cessant d'être un angle mort. |
| `P2/FOR-5` | ✅ — aucune date fériée en dur | Confirmé et **rendu opposable** par L2-3 ; surtout, un défaut réel corrigé au passage : `load_mg_holidays`, référencée par la docstring du calendrier, **n'existait pas**. `ForHoliday` n'était peuplée par rien, donc `business_days_in_month` surestimait la capacité de production d'une dizaine de jours par an. |
| `P1/SAL-5` | ✅ — le taux de TVA vient d'`AccTax` | Confirmé et rendu opposable par L2-3. **Écart fonctionnel signalé au passage, hors critère** : `sales/services/orders.py` pose `amount_tax = Decimal(0)` — le module Sales ne calcule aucune taxe, seul le POS applique `get_default_sale_tax`. SAL-5 porte sur l'origine du taux, pas sur son application ; l'écart reste réel et est versé au lot L5. |
| `P3/STK-6` | ✅ à l'audit — comptage à l'aveugle | **Réserve levée par L12-3** : L13, en rendant le mode explicite (`is_blind`), l'avait rendu optionnel et l'avait mis à `False` par défaut — tout inventaire créé sans rien dire révélait la quantité théorique. Le défaut est revenu à `True` et c'est l'affichage qui demande désormais une action explicite. |
| §3.4 `apps/quality` est inatteignable depuis le produit | ❌ | Fermé par **L11** : `views.py`, `urls.py`, `api.py`, montage dans `config/urls.py` et `config/api.py`, entrées RBAC pour les cinq rôles de `_QLT_FULL_ROLES`, lien de barre latérale et tuile de launchpad. Le module publie en outre ses quatre premiers évènements (`quality.non_conformity_opened`, `_closed`, `quality.recall_declared`, `_closed`) : il n'en publiait aucun, et n'était donc automatisable par rien. |
| §3.6 Un secret de tiers est stocké en clair | ❌ — deux champs signalés | Fermé par **L15**, et l'audit en sous-comptait un : `LogServiceProvider.webhook_secret` est chiffré (`EncryptedCharField`), `PrjGuestAccess.token` et **`UserEmailChangeRequest.token`** — troisième, trouvé en fermant les deux autres — ne stockent plus qu'une empreinte SHA-256. Une garde d'architecture ferme désormais la classe entière, et vérifie au passage que `object_remap.SECRET_TOKEN_FIELD_NAMES` couvre bien les nouveaux noms de champ : le passage de `token` à `token_hash` l'avait rendu muet, ce qui aurait fait partager le même credential à deux tenants après import d'archive. |
| §3.3 Dérive documentaire | 🟡 | Fermé par **L15** pour les deux docstrings restantes (`stocks/services/consistency.py`, `sales/services/reports.py` — celle-ci affirmait encore que « `apps.stocks` n'existe pas encore »), et par **L0-4** pour les dix-huit docstrings de commandes qui affirmaient « aucun mécanisme de cron n'est câblé ailleurs ». |
| `P3/STK-12` | 🟡 — « l'égalité est une conséquence attendue de la conception, non une propriété vérifiée » | Fermé par **L12-1**, et la vérification a trouvé ce que la conception cachait : **l'égalité ne pouvait pas tenir**. `create_stock_movement_entry_from_source` choisissait le compte par le SIGNE de la ligne — positif au compte de stock, négatif au compte de variation — ce qui rendait littéralement impossible de **créditer** le compte de stock. Toute sortie (livraison, vente au comptoir, perte d'inventaire) le débitait donc comme une entrée : le solde croissait de la somme de tous les mouvements au lieu de suivre la valeur détenue, et la contrepartie de charge était créditée au lieu d'être débitée. Le compte choisit désormais par un `account_role` explicite, le signe ne dit plus que le sens. Un rejeu en mémoire depuis les `StkMove` (`stocks/services/valuation_replay.py`, qui ne lit jamais `StkValuationLayer`) prouve l'égalité, y compris à chaque date intermédiaire. |
| `P3/PRD-9` | 🟡 — « aucun test ne vérifie l'égalité à l'ariary près avec la somme rejouée » | Fermé par **L12-2**, et là encore la vérification a trouvé un écart : le coût était valorisé au **CUMP courant**, celui du jour de la clôture, quand le critère exige celui de la **date d'effet** de chaque consommation. Sur un ordre dont les consommations s'étalent et dont le CUMP bouge entre-temps, le coût de revient était celui d'un approvisionnement qui n'avait pas servi à le fabriquer — mesuré à 30 000 Ar au lieu de 10 000 sur le scénario de test. La date d'effet n'était d'ailleurs **enregistrée nulle part** : ni `consume_component` ni `record_component_consumption` ne la posaient. `MrpOrderComponent.consumed_at` la porte désormais, et `close_order` résout le coût par `stocks.services.public.get_variant_unit_cost_at_date` (le rejeu de L12-1). Repli **journalisé** sur le CUMP courant pour les composants antérieurs au champ : les écarter du coût les compterait pour zéro. Les ordres déjà clôturés gardent leur coût — discipline « document validé immuable ». |
| `P3/ACH-10` | 🟡 — « aucun test ne compare les deux restitutions à l'ariary près » | Fermé par **L12-3**, après **correction du périmètre du critère** : le rapprochement littéral (`AnFactReception.cout_debarque_unitaire_mga` contre le moteur de valorisation) est une **tautologie** — ce champ est alimenté par le retour littéral de `get_variant_unit_cost`, c'est-à-dire par le moteur lui-même ; le test existant pose une couche à 1200 et assert 1200. Le rapprochement qui a un sens oppose le coût débarqué **calculé** par `landed_cost_report` au coût **valorisé** après `apply_landed_cost_to_valuation`. Une limite réelle est testée au passage plutôt que laissée en prose : dès qu'une partie du stock est sortie avant la finalisation du lot, les frais se répartissent sur la seule quantité restante et les deux chiffres divergent légitimement. |
| `P3/PAY-10` | 🟡 — « aucun test comparant la somme des rubriques au total du journal » | Fermé par **L12-3**. `services/batches.py` affirmait « vérifié algébriquement équilibré **par construction** » — jamais testé, et le test existant n'assertait que `total_debit == total_credit`, ce qu'une écriture équilibrée **sur un mauvais montant** satisfait aussi. Les nouveaux tests remontent aux `PayPayslipLine`, et l'un d'eux fausse deux totaux de façon **compensée** : l'écriture reste parfaitement équilibrée, l'ancienne assertion passe toujours, seule la comparaison aux rubriques voit l'erreur. |
| `P3/PRD-6` | 🟡 — calculé depuis les déclarations, mais non recalculable depuis les mouvements | Fermé par **L12-4**, et la seconde moitié du critère était **infaisable**, pas seulement non testée : `declare_scrap` annonçait que le mouvement vers l'emplacement rebut « sera branché […] une fois ces modules disponibles » alors que `mrp` consommait déjà `stocks.services.public` depuis A2. **Aucun `StkMove.TYPE_REBUT` n'existait dans le dépôt** — le rebut n'atteignait jamais le stock. Les deux natures de rebut produisent désormais leur mouvement sous des `source_document` distincts (`{réf}/WO{poste}` et `{réf}/SCRAP`) : les confondre fait tomber le taux recalculé à 73,17 % au lieu de 83,33 %, mesuré. Le mouvement ne déplace aucun montant — vérifié contre le rejeu de STK-12, pas affirmé. |
| `P3/STK-10` | 🟡 — retour visuel de scan sous 300 ms, jamais mesuré | Fermé par **L12-5**, avec un défaut trouvé en le mesurant : `offline_queue.js` cherche `.wh-toast-container` et, faute de le trouver, retombe sur un **`window.alert()` bloquant**. Ce conteneur n'existait que dans `base.html` — les deux gabarits autonomes qui chargent la file hors-ligne (`stocks/tw-scan.html`, `tw-launchpad.html`) n'en avaient aucun. Chaque scan hors connexion ouvrait donc une boîte de dialogue modale à fermer avant l'article suivant, sur l'écran conçu pour le mode dégradé. Le seuil est désormais mesuré hors ligne ET serveur coupé — ce qui s'affiche quand le serveur ne répond jamais est nécessairement posé côté client, ce qu'une mesure en ligne ordinaire ne distinguerait pas. |
| `P2/BI-1` | 🟡 — formule non exécutable, deux tables codées en dur, test de cohérence introuvable | Fermé par **L8**, après avoir trouvé pire que ce que l'audit décrivait : le dictionnaire n'était **peuplé par rien** — ni migration, ni commande, ni chemin de création de tenant. Sur une instance neuve il était vide, donc l'écran était vide, donc aucun rapport ne pouvait nommer un indicateur. `METRIC_FACTS` est supprimé : le fait vit sur l'indicateur (`fait_source`), validé à l'enregistrement contre les faits réellement exposés, et `axes_autorises` validé contre les axes de ce fait. Un indicateur créé à l'exécution est donc calculable **sans déploiement**. Un jeu de huit indicateurs publiés est chargé aux quatre chemins de création de tenant, et une migration rattrape les tenants existants. Le test de cohérence entre deux écrans existe désormais (`bi/tests/test_metric_consistency.py`), avec sa contre-épreuve. |
| `P2/BI-2` | 🟡 — « la garde CI exigée n'existe pas » | **Déjà fermé par L2** (`tests/architecture/test_no_raw_sql_in_reporting.py`), l'audit ne l'avait pas encore intégré. **L8** ferme la seconde moitié : le constructeur n'acceptait que des mesures et dimensions déclarées, mais rien ne vérifiait qu'une dimension déclarée soit réellement produisible — un axe fantôme passait l'enregistrement et se perdait en silence à la première consultation. |
| `P2/BI-4` | — | **Défaut collatéral corrigé par L8** : `run_report` écartait silencieusement tout indicateur inconnu ou non calculable, là où `drill_down` répondait déjà « Indicateur non calculable » sur la même condition. Un tableau de bord auquel il manque une ligne sans qu'un mot l'explique se lit comme un tableau complet. |
| `P2/STR-1` | 🟡 — « la création n'est donc pas refusée comme l'exige le critère » | Fermé par **L8**, et le vrai problème était ailleurs : la garde existait, fonctionnait, et était **insatisfiable en production**. `activate_objective` exige un résultat clé adossé à un `metric_code` publié ; le dictionnaire étant vide sur toute instance réelle, aucun code n'existait jamais, donc **aucun objectif ne pouvait être activé**. Les tests la vérifiaient en enregistrant eux-mêmes l'indicateur dont ils avaient besoin. Un critère vert sur une fonctionnalité inaccessible. Les nouveaux tests partent d'un tenant créé par le chemin réel, sans rien enregistrer. |
| `P3/PAY-9` | 🟡 — réserve assumée : « recopie et recalcul complets plutôt qu'un delta, rapprochement à la charge du gestionnaire de paie » | Fermé par **L14**, et la réserve cachait un défaut d'argent. Le bulletin rectificatif est un `PayPayslip` ordinaire à l'état `computed` : `create_batch` le ramasse dans le lot de la période cible, et **rien nulle part ne lisait `rectifies`**. Corriger une erreur de 57 692 Ar sur un bulletin de 1 200 000 Ar postait donc une écriture de **1 257 692 Ar** et ordonnait un **second virement complet** au salarié — mesuré, en rétablissant temporairement l'ancien comportement. Les trois chemins d'argent (totaux du lot, écriture, fichiers de paiement) passent désormais par `regularization_movement`, qui ne bouge que l'écart. Le bulletin garde ses valeurs pleines : un document remis à un salarié doit porter son salaire réel. Un écart **négatif** (trop-perçu) est conservé tel quel — le ramener à zéro ferait disparaître une dette. |
| `P3/PRD-5` | 🟡 — « le fonctionnement sur tablette en réseau dégradé n'est vérifié par aucun test » | Fermé par **L14**, et le test a trouvé bien plus que ce qu'il cherchait. Le geste hors ligne est bien mis en file, confirmé par un toast non bloquant, et rejoué au retour du réseau — mais l'ordre de travail avançait alors avec **qté produite = 0**. |
| §3.7 Nombres localisés dans les champs `type="number"` | non relevé par l'audit | **Défaut systémique trouvé par le test PRD-5.** Sous locale française, Django rend un `Decimal` avec une **virgule** (`value="5,0000"`) ; le navigateur juge la valeur invalide pour un `type="number"` et la lit comme une **chaîne vide**. Sept champs étaient touchés. Conséquences vérifiées : chaque « Terminer l'étape » du kanban atelier enregistrait **0 produite et 0 rejetée** (donc un FPY calculé sur des zéros) ; enregistrer une fiche partenaire **effaçait son encours autorisé** ; enregistrer la configuration WhatsApp **effaçait le plafond mensuel de coût**. Aucun test ne pouvait le voir — les tests HTTP postent des valeurs qu'ils composent eux-mêmes, jamais celles qu'un navigateur aurait lues. Corrigé par `|unlocalize`, et fermé par une garde qui a immédiatement trouvé un septième champ que la recherche manuelle avait manqué. |
| `P1/SAL-8` | 🟡 — « le paramétrage par tenant des mentions obligatoires n'a pas été retrouvé » | Fermé par **L5**, et ce n'était pas le paramétrage qui manquait : **c'était la facture**. `invoice_pdf` concaténait une f-string de douze lignes, sans raison sociale, sans adresse, sans NIF de l'émetteur, sans identité du client (`partner_id` n'était même pas lu), sans ventilation HT/TVA/TTC — dans un document déclaré `is_legal_document=True`, donc archivé et immuable, atteignable en deux clics et par l'API. Les libellés y étaient interpolés **sans échappement**. Le document passe désormais par le gabarit légal partagé (échappement par construction), résout le nom du client, ventile HT/TVA/TTC depuis les comptes, applique la règle unique de présentation de l'Ariary et rend les mentions légales du tenant (`Tenant.legal_mentions`, champ nouveau). |
| `P2/WA-1`, `P2/WA-2` | 🟡 — contrôle du consentement à l'envoi acquis | **Deux défauts qui se renforçaient**, fermés par **L10**. (1) `record_inbound_whatsapp_message` n'acceptait aucun tenant : tout message entrant était écrit avec `tenant_id = NULL`, donc **invisible de tous les écrans**, alors que le webhook avait résolu le tenant douze lignes plus haut. (2) « STOP » ne désabonnait personne — `handle_inbound_message` ne reconnaissait que « 1 », « 2 », « 3 » et n'importait même pas `revoke_consent`. Ensemble : le client demandait à être désabonné, personne ne le voyait, et les campagnes continuaient. Le désabonnement coupe désormais court — répondre à « STOP » par un menu d'intentions serait exactement le message refusé. |
| `P2/WA-7` (partie exploitation) | 🟡 | **`WHATSAPP_DEFAULT_TENANT_ID` n'était semé dans aucun fichier de déploiement** : un exploitant qui suit `.env.example` branche le webhook et obtient `{"governed": false}` pour toujours — module installé, monté, testé, et inerte. Ajouté à `.env.example` et à `DEPLOYMENT_HETZNER.md`. |
| `P1/SAL-1` | 🟡 — « ni la navigation clavier exhaustive ni le chronomètre ne sont vérifiés par un test » | Fermé par **L16**, et il ne manquait pas une mesure : **le parcours était impossible**. Le seul moyen de renseigner `partner_id` — champ obligatoire côté vue — était un `<li>` nu, ni focalisable ni actionnable au clavier, dont l'unique gestionnaire était un `click` délégué. Aucun commercial travaillant au clavier ne pouvait créer de devis, quelle que soit la durée. Le trou était invisible en CI : le parcours e2e existant contourne le composant par `page.evaluate`. Chaque résultat est désormais un vrai `<button>` — focalisable, actionnable par Entrée, couvert par le même gestionnaire sans une ligne de JS supplémentaire — avec un focus visible. Trois tests e2e **sans aucun `page.click`**. |
| `P1/ACC-3` | 🟡 — « le parcours clavier intégral et l'affichage de l'équilibre en continu n'ont pas été vérifiés » | **Défaut trouvé sous le critère** : `post_move` ne vérifiait que l'équilibre débit/crédit. Zéro égale zéro, donc **une écriture SANS AUCUNE LIGNE passait le contrôle**, consommait un numéro de la séquence légale (RG-ACC-3) et devenait immédiatement immuable par déclencheur base. L'écran affichait même un badge vert « Équilibrée » sur ce brouillon vide : un comptable qui tabulait jusqu'au premier bouton publiait une pièce numérotée vide. Un trou dans une numérotation légale ne se répare pas. Garde dans le service, en amont de l'attribution du numéro ; l'écran cesse de la contredire. |
| §3.8 Commentaires de gabarit rendus dans le HTML | non relevé par l'audit | **Défaut systémique, 24 occurrences.** Django ne supprime PAS un commentaire `{# … #}` qui s'étend sur plusieurs lignes : son texte est rendu tel quel dans le HTML envoyé au navigateur, et toute balise qui y est citée devient un **vrai élément**. Le dépôt l'avait déjà rencontré une fois (`templates/reports/_base.html`, fuite jusque dans un PDF) et documenté — puis répété 24 fois. Trouvé une seconde fois par le test de parcours clavier SAL-1 : un `<button>` cité dans un commentaire capturait le focus avant le vrai résultat de recherche. Tous convertis en `{% comment %}`, et fermé par une garde. |
| `P1/SAL-2`, `P1/SAL-3`, `P1/SAL-5` | ✅ / ✅ / 🟡 — « chaîne réellement câblée de bout en bout » | **Défaut trouvé sous trois critères verts ou orange : la chaîne était câblée, mais sans TVA.** `sales` portait `tax_id` sur ses deux modèles de ligne depuis S1 — jamais renseigné par aucune surface d'entrée (ni API, ni vues, ni seeds) — et les deux `_recompute_totals` écrivaient `amount_tax = Decimal(0)` **en dur**. En bout de chaîne, `invoice_order` ne transmettait à `accounting` que des lignes hors taxe : **aucune facture émise depuis une commande de vente n'a jamais crédité de compte de TVA collectée**, et le PDF de facture (fermé la veille par L5/SAL-8, qui ventile HT/TVA/TTC depuis les comptes) affichait donc `TVA : 0 Ar` sur chacune d'elles. Le module qui facture les clients était le seul du produit à ne pas savoir facturer la taxe — le POS, lui, l'appliquait depuis sa première version. Fermé par **L5** : `sales.services.taxes` fige le taux par ligne à la date du document (`tax_rate`, nouveau champ, même rôle que `PosOrderLine.tax_rate`), `invoice_order` émet une ligne de TVA collectée par taxe sur la base réellement facturée, et `AccMoveLine.tax`/`tax_base` — deux champs que le modèle portait depuis l'origine et qu'aucun écrivain ne renseignait — trouvent leur premier écrivain. Aucun test existant n'a rougi : **aucun jeu de données du dépôt ne créait d'`AccTax` de vente sur le chemin `sales`**, ce qui rendait l'absence de TVA structurellement invisible. |
| `P1/SAL-5` (suite) et RG-ACC-5 | 🟡 | **Deux défauts de plus, trouvés en branchant le premier.** (1) `vat_applicable` ne comparait qu'au régime synthétique : elle déclarait assujetti le régime `reel_sans_tva`, le seul des trois dont le libellé dit littéralement « Réel, **sans** assujettissement TVA ». Les trois régimes étaient traités comme deux, et le test ne couvrait que les deux extrêmes. (2) `public.get_default_sale_tax` — la surface réellement consommée par le POS, et désormais par `sales` — ne lisait **jamais** le régime fiscal, alors que `applicable_taxes` le masquait correctement : les deux surfaces se contredisaient, et c'est celle qu'on appelle qui avait tort. Au passage, `Tenant.vat_opted_in` (ACC-SMT1, option d'assujettissement de la Loi de Finances 2026), documenté sur quinze lignes, n'était lu par **rien** dans tout le dépôt ; il trouve ici son premier lecteur — et son unique effet légitime. |
| `P2/WA-1`, `P2/WA-2` (suite) | fermés par L10 côté webhook gouverné | **Une seconde porte restait ouverte, et elle réintroduisait le défaut qu'on venait de fermer.** `apps/core/api_notifications.py` exposait toujours `/notifications/whatsapp/webhook`, monté et joignable, conservé « par compatibilité ascendante » — il appelait `record_inbound_whatsapp_message` **sans `tenant_id`** (donc messages invisibles de tous les écrans) et n'appelait jamais `handle_inbound_message` (donc « STOP » ignoré, aucune conversation ouverte, aucun menu envoyé). Une compatibilité ascendante qui rétablit intégralement le défaut réparé à côté n'en est pas une : la porte est retirée, et une note de migration d'exploitation dit comment repointer l'URL Meta. |
| `P1/CRM-6` (RG-CRM-5) | ✅ — « scoping serveur, vue **et** API » | **Le verdict était vrai de la liste, faux du reste.** `apps/crm/views.py` repasse par `scope_leads_for_user` et le commente en quinze lignes ; `api.py::list_leads` aussi. Mais **quatre** endpoints du même fichier faisaient `get_object_or_404(CrmLead, id=lead_id)` tout court : déplacement d'étape, activités en lecture et en écriture, forçage de remise. Tout porteur de `crm.change_crmlead` dans le tenant — un commercial qui ne voit que ses propres opportunités à l'écran comme à la liste — pouvait, par simple connaissance de l'UUID, déplacer l'opportunité d'un collègue, la marquer gagnée ou perdue, y ajouter une activité. Ce n'était pas une fuite entre tenants (elle tient par RLS) mais le périmètre **interne**, celui que RG-CRM-5 définit. Même motif que la TVA la veille : la règle tenue sur une surface, oubliée sur l'autre. 404 et non 403, comme la vue web et comme les bulletins de paie. |
| `P1/SAL-5` / RG-ACC-5 (volet « proposée ») | 🟡 | La règle dit « aucune `AccTax` n'est **proposée ni appliquée** ». Seul « appliquée » était tenu, et uniquement par trois surfaces de lecture. L'écran `config_taxes` créait une `AccTax` sur un tenant synthétique sans le moindre avertissement, et listait par `AccTax.objects.filter(...)` direct plutôt que par `applicable_taxes()` ; le modèle ne porte la règle qu'en commentaire, sans `clean()` ni contrainte. Un tenant synthétique pouvait donc se constituer un jeu de taxes que le produit refuserait ensuite d'appliquer — une configuration sans effet, et rien pour le dire. Refus **côté POST**, jamais seulement dans le gabarit. |
| Régime de l'Impôt Synthétique (question du maître d'ouvrage) | non relevé par l'audit | **La TVA livrée la veille était juste au calcul, fausse à l'affichage.** Un tenant non assujetti ne collectait rien nulle part — mais la facture légale imprimait sans condition « Total HT / TVA : 0 Ar / Total TTC », sur un document `is_legal_document=True` donc archivé et immuable. « TVA à zéro » affirme qu'une taxe existe et vaut zéro ; « TVA non applicable » dit que le document n'entre pas dans le champ de la taxe. Et « HT »/« TTC » opposent deux montants identiques. Le ticket POS remis au client et l'écran de caisse portaient le même défaut, sans condition eux non plus — et les gabarits `sales` corrigés la veille avaient copié leur ventilation depuis ce ticket-là, c'est-à-dire depuis la surface non corrigée. Fermé par **L17** sur les sept surfaces, écran et PDF confondus. |
| Le vrai défaut sous-jacent | — | **Aucune surface d'affichage ne lisait `vat_applicable`.** Mes gabarits de la veille conditionnaient sur `amount_tax` — un **montant** — et non sur le **régime** : ils masquaient donc la même ligne pour un non-assujetti (normal) et pour un assujetti dont personne n'a configuré d'`AccTax` (un gap de configuration). Les deux surfaces du même devis divergeaient d'ailleurs : le PDF masquait « Total HT » mais imprimait « Total TTC », l'écran faisait l'inverse. Nouveau gap public `is_vat_liable`, plus un processeur de contexte, et le doute disparaît. Au passage, `sales.services.taxes.resolve_line_tax` cesse de journaliser un repli normal : le bruit permanent d'un tenant synthétique disparaît, et un tenant **assujetti** sans `AccTax` devient un `warning` au lieu d'un `info` noyé. |
| Quatre défauts trouvés en instruisant la question | — | (1) `tva.seuil_assujettissement` et `tva.taux_export`, déclarés par le cahier des charges dans l'amorçage de la Phase 1, n'existaient **dans aucun fichier du dépôt** — semés. (2) `fiscal_regime`, `vat_opted_in` et `legal_mentions` n'étaient écrits par **aucune surface du produit** : tout tenant naissait « réel, avec TVA », et seul `/admin/` permettait de corriger — l'écran de configuration fiscale que la docstring de `vat_opted_in` annonçait depuis l'origine existe enfin, et **signale** un régime incohérent au vu du chiffre d'affaires sans jamais le changer d'office. (3) `clone_tenant_to_sandbox` ne recopiait pas le régime : le bac à sable d'une entreprise à l'impôt synthétique était un tenant assujetti. (4) `seed_default_tax_calendar` posait à un tenant synthétique une échéance « TVA mensuelle » et une échéance IRCM — cette dernière étant **refusée** par `generate_ircm_declaration` : le calendrier promettait une déclaration que le produit refuse de produire. |
| `P1/SAL-2` (suite) | — | **`sales` était le seul écrivain de `tax_lines`.** Un tenant parfaitement assujetti qui facturait une régie de projet ou une refacturation de fret émettait encore une facture sans TVA. Les deux chemins automatiques (`projects`, `logistics`) la portent désormais, au taux de vente par défaut du tenant — même simplification assumée que le POS, faute de sélecteur de taxe par ligne. **Correction d'une affirmation du plan** : `invoice_import` n'était PAS concerné, il ajoutait déjà sa ligne de TVA. Restent hors périmètre, et c'est dit : la création manuelle de facture (écran et API `accounting`) n'expose aucun champ de TVA — c'est une surface de saisie à faire évoluer, pas un service à corriger. |
| Ce que L17 n'a **pas** fermé | — | Deux surfaces de **saisie manuelle** de facture (`accounting/views.py`, `accounting/api.py`) n'exposent aucun champ de TVA : un comptable qui crée une facture à la main y saisit un compte de produit et un montant, sans possibilité d'ajouter une ligne de taxe. Ce n'est pas un service à corriger — `create_invoice` accepte `tax_lines` depuis L5 — mais un formulaire et un schéma d'API à faire évoluer. Chiffré séparément, et dit ici plutôt que laissé à découvrir. |
| Simulation et régime | — | `baseline.build_baseline` **levait** une `ValidationError` faute de `tva.taux_normal`, en contradiction frontale avec `vat_reference.resolve_reference_vat_rate` qui pose depuis L3 qu'« un tenant peut légitimement n'avoir aucun référentiel de TVA (régime synthétique) ». Deux modules affirmaient l'inverse l'un de l'autre : **un tenant synthétique ne pouvait construire aucun socle de simulation**. Le taux reste exigé d'un tenant assujetti. |
| `P1/CRM-1` | ❌ | Fermé par **L4**. Aucun kanban n'existait dans `apps/crm` ni `templates/crm` : le changement d'étape se faisait par un `<select>` depuis la **fiche**, jamais depuis le pipeline — alors que `Sortable.min.js` était vendorisé et utilisé par `mrp` et `projects`. Patron de `templates/mrp/kanban.html` repris entier, doctrine comprise : le glisser-déposer est cosmétique, le bouton reste le seul chemin accessible au clavier. Le **chatter** n'était câblé sur aucun objet CRM ; il l'est, avec sa garde de périmètre — sans elle, le repli de `core.views.chatter` est la permission de modèle `crm.view_crmlead`, que tout commercial porte, et le fil de discussion (notes internes, motifs de perte) de l'opportunité d'un collègue devenait lisible et écrivable. Le périmètre RG-CRM-5 est appliqué au kanban lui-même. |
| `P1/CRM-2` | ❌ | Fermé par **L4**. La fiche société affichait déjà devis, commandes et grand livre — mais **derrière l'onglet du rôle « client »**, alors que le critère exige « sans navigation supplémentaire », et vingt par vingt au lieu de trois. Ni l'encours ni le solde comptable n'existaient nulle part, et **aucun des deux n'avait de gap public** : l'encours n'était même pas une fonction, il vivait dans le corps de `sales.services.orders.confirm_order` — seule définition écrite du dépôt, désormais extraite et republiée plutôt que recopiée. Le solde par tiers rejoint `get_stock_account_balance` comme second agrégat du contrat public d'`accounting`. Les deux montants sont affichés côte à côte avec la note qui les sépare : une commande facturée quitte l'encours pour le solde, ils ne s'additionnent pas. |
| `P1/CRM-3` | ❌ | Fermé par **L4**. Aucune fonction de conversion n'existait : `CrmLead.partner_id` était un UUID nu que quelqu'un remplissait à la main après avoir créé la société ailleurs, en recopiant nom, e-mail et téléphone déjà saisis. « Sans ressaisie » était immédiat — la piste porte déjà ces champs, il n'y avait qu'à les transmettre. Nouveau gap `partners.services.public` composant `create_partner` et `create_contact` existants. Précision assumée : dans ce modèle, `CrmLead` **est** l'opportunité (elle porte pipeline, étape et montant dès sa création) ; la conversion rattache la société à l'opportunité existante plutôt que d'en fabriquer une seconde. |
| `P1/CRM-4` | 🟡 — « ce n'est pas une tuile de launchpad, et le seuil N n'est pas un paramètre » | Fermé par **L4**, et l'audit visait juste sur les deux points. `_STAGNANT_WINDOW_DAYS = 21` était une constante de module : aucun tenant ne pouvait l'ajuster sans une livraison, alors qu'un cycle de vente long et un cycle court n'ont pas la même notion de « en retard ». Le seuil est porté par `CrmPipeline` (c'est lui qui décrit le cycle), éditable à l'écran de configuration, et **lu par une définition unique** partagée avec le détecteur d'anomalies du copilote — deux comptages de la même population feraient diverger la tuile et la liste, ce que BI-1 interdit ailleurs. |
| `P1/CRM-5` | ❌ | Fermé par **L4**, partiellement et c'est dit. Le composant d'état vide générique n'existait dans aucun des 8 fichiers de `templates/components/` ni des 6 de `templates/cotton/` ; le vide retombait sur « Aucun résultat. » dans une cellule de tableau. `<c-empty-state>` distingue les deux vides — rien du tout, ou un filtre trop fin —, ce dernier gardant le tableau et sa barre de recherche. **La moitié « import d'un fichier » du critère n'est pas livrée** : aucun import CRM n'existe dans le dépôt, et pointer vers un import inexistant serait pire que de ne rien proposer. À chiffrer séparément. |
| Recherche de la liste CRM | non relevé par l'audit | **Défaut trouvé par un test de falsification de CRM-5.** `Column(key="stage")` était marquée cherchable, or `stage` est une clé étrangère : `_apply_search` y construisait un `stage__icontains` et Django levait `FieldError` — **500 dès la première frappe dans la barre de recherche de la liste des opportunités**. Une seule occurrence dans tout le dépôt (vérifié par introspection des 22 groupes de colonnes). `Column.search_key` permet d'afficher la relation tout en cherchant sur une colonne texte, plutôt que de supprimer une recherche par étape qu'un utilisateur attend. Fermé par une garde d'architecture avec auto-test. |
| `P1/IA-2` | 🟡 | **Défaut grave fermé par L7 : l'audit disait le contraire de ce qui s'était passé.** `data_query_gateway.ask()` faisait `except AIProviderError: tools_called = []` — détruisant la liste construite dans la boucle, alimentée **après** l'exécution réelle de chaque outil. Au tour 1 le LLM appelle `sales.revenue_report`, l'outil lit vraiment le chiffre d'affaires du tenant ; au tour 2 le fournisseur tombe ; l'`AiDataQuery` est persistée avec `tools_called=[]`. Et un test existant **assertait ce comportement**, avec un fournisseur qui échouait au *premier* appel — cas où `[]` est correct : il ne distinguait pas « aucun outil » de « outils perdus ». Corrigé structurellement — la liste appartient désormais à l'appelant et la boucle l'alimente en place, de sorte qu'aucun chemin d'exception ne puisse plus la réaffecter. La docstring de la boucle promettait par ailleurs « jamais d'exception propagée » alors qu'aucun `try` n'entourait l'appel au fournisseur. Le champ `duration_ms` manquait aussi : les sept migrations du module n'en contenaient pas une occurrence. |
| `P1/IA-4` | 🟡 | Fermé par **L7**. Les six tests de la passerelle ne manipulaient qu'**un seul** tenant : ils ne pouvaient rien dire du critère. Le test à deux tenants nomme explicitement l'autre société dans la question et vérifie que l'outil n'est jamais invoqué pour un autre tenant que l'appelant, et que l'enregistrement d'audit reste dans le tenant d'origine. |
| `P1/IA-7` | 🟡 | Fermé par **L7**. Une phrase unique — « Analyse incomplète (fournisseur IA non configuré, budget épuisé, ou erreur) » — couvrait trois causes et n'en désignait aucune. Un dépassement de délai n'appelle pas la même conduite : le service répond, il est lent, et réessayer a du sens. Nouvelle exception `AIProviderTimeoutError` (sous-classe, donc tout appelant existant continue de fonctionner) et message d'attente distinct. |
| `P1/IA-8` | ❌ | Fermé par **L7**. Le bloc « Sources consultées » existait déjà — en `<strong>` : il nommait le rapport sans jamais y mener. Nommer la source dit d'où vient le chiffre, cela ne permet pas de le recontrôler. `DataQueryTool` porte désormais un `verification_route` (nom de route, jamais une URL en dur qui deviendrait un lien mort), et les huit outils enregistrés le déclarent. Dégradation propre : un outil sans écran de vérification, ou dont la route a disparu, reste nommé sans lien. |
| `P1/IA-9` | 🟡 | Fermé par **L7**. Deux tiers du critère étaient tenus par accident : sans `AI_PROVIDER_CONFIG`, le stub répond et rien ne sort. Mais l'**activation** était une variable d'environnement posée au redémarrage — aucun écran, aucun modèle, aucune entrée d'audit : personne ne pouvait dire depuis quand ni par qui les données d'une société sortaient. Le consentement est désormais **par tenant** (`AI_PROVIDER_CONFIG` dit ce que l'hébergeur a branché, pas quelle société l'accepte), enregistré avec le texte exact présenté, et une révocation date la ligne au lieu de l'effacer. Sans consentement, retour au stub. |
| `AI_PROVIDER_CONFIG` inconfigurable | non relevé par l'audit | **`AI_PROVIDER_CONFIG` était un dictionnaire vide en dur.** Aucune variable d'environnement ne l'alimentait, et `.env.example` n'en soufflait mot : un exploitant ne pouvait brancher aucun fournisseur d'IA sans **modifier le code source**. Le connecteur était écrit, testé et documenté — et hors de portée. Même motif que les paramètres réglementaires semés nulle part (L3, L17). Lu depuis l'environnement et documenté. |
| `P1/IA-1` | 🟡 — les outils du copilote étaient tous en lecture, mais rien ne le disait | Fermé par **L2-1** : `read_only` est un paramètre obligatoire de `register_data_query_tool`, le registre refuse un outil d'écriture, et la garde vérifie aussi que chaque `required_permission` est bien en `<app>.view_`. |
| `P2/BI-4` (le critère lui-même) | 🟡 — « toute l'information exigée est affichée, mais **sur la page d'index de BI**, pas sur chaque tableau de bord » | Fermé par **L9**, et l'audit était encore optimiste : `get_latest_refresh_summary` n'avait qu'**un seul appelant**, la branche `tab=journal` — sa propre docstring citait pourtant le critère mot pour mot (« visible sur chaque tableau de bord »). L'état est désormais dans le contexte de base de la vue, donc sur les quatre onglets, et le bandeau rend les **quatre** faits que le critère nomme : dernière exécution, durée, volume, échec (écart de réconciliation **et** message d'erreur — la durée et le message manquaient au gabarit). |
| `P2/BI-*` (l'onglet par défaut) | non relevé par l'audit | **Le premier écran de tout utilisateur du module était structurellement vide.** `BiDashboard` avait six occurrences dans le dépôt — sa définition, sa migration, un import, une lecture dans la vue, et une `BiDashboardFactory` **jamais utilisée** : aucun `objects.create`, aucune route d'écriture, aucune commande, aucune fixture, aucun admin. Or `dashboards` est l'onglet par défaut de `/bi/` : chacun y lisait « Aucun tableau de bord pour votre rôle », sur toute instance. Aucun test n'échouait, puisque aucun n'en créait. L9 livre la voie d'écriture qui manquait (`services/dashboards.py`, écran `dashboards/new/`), un jeu de départ idempotent par rôle sur le patron de L8, et une validation des tuiles qui **refuse le rapport d'une autre société** — `tiles` étant un JSONField dénormalisé, un tableau dont une tuile ne résout pas s'affichait silencieusement amputé. |
| `P2/FOR-7` | 🟡 — « la mesure est écrite et correcte mais n'est appelée que depuis son propre test » | Fermé par **L9**. Le rapprochement des périodes échues a lieu à la fin de `compute_and_store_forecast`, là où le réalisé est déjà chargé. L'audit avait raison sur la conséquence : l'onglet qualité filtre `statistical_error_pct__isnull=False` et restait **vide en exploitation** — le critère n'était pas seulement non mesuré, il était infaisable. |
| `P2/FOR-9` | 🟡 — « le délai retenu est la moyenne tous clients confondus » | Fermé par **L9**, et le blocage n'était pas celui que l'audit décrivait. Remplacer la formule n'aurait rien changé : les séries lues étaient filtrées sur `DIMENSION_CANAL`, qui ne porte **aucun client**. La projection lit désormais les prévisions par client quand elles existent (`assumption_basis="par_client"`), et à défaut pondère le délai par le **nombre de ventes observées** au lieu d'une moyenne plate (`"canal_pondere"`) — la base retenue étant nommée dans le résultat, jamais implicite. `get_partner_payment_behavior` ne publiait pas `sales_count` : sans cet ajout, la pondération aurait dégradé en silence vers l'ancienne moyenne plate. |
| `P2/FOR-10` | ❌ — « aucun chemin ne fait entrer une prévision publiée dans `apps/simulation` » | Fermé par **L9**, sur le patron que `strategy` appliquait déjà depuis son propre chantier (`services/budget.py`) : le socle porte la **version et la date** de la publication qui l'alimente, pas un chiffre anonyme. `simulation` déclare désormais `forecast` en dépendance. |
| `P2/BI-3` | ❌ | **Reste ❌ — décision du maître d'ouvrage, pas un oubli.** Le critère demande de reconstruire « les rapports retenus à l'issue de la rationalisation » du catalogue **hérité** (91 rapports du système existant du client) et de les rapprocher à l'ariary près. Ce catalogue n'est pas dans le dépôt : produire un arbitrage « conserver / fusionner / paramétrer / supprimer » sur un inventaire qu'on n'a jamais vu serait un faux, et un vert fabriqué coûte plus cher qu'un rouge assumé. Livré à la place, ce que le même paragraphe du cahier demande (H6, risque P2-R2 « porter le catalogue tel quel industrialise l'incohérence ») : un **budget de rapports vérifié en CI** (`tests/architecture/test_report_budget.py`), qui borne la dérive du catalogue et exige que chaque rapport déclare son module, son libellé et sa permission. Il ne dit pas que le catalogue est cohérent ; il dit qu'il ne grossit plus sans décision. Au passage, un chiffre de cet audit est corrigé : le dépôt compte **63** rapports réellement enregistrés, pas 85 — les 85 étaient des occurrences de `register_report(` dont un quart vivent dans des tests. |
| `P2/WA-7` | 🟡 — « l'envoi est synchrone (aucune file) et la reprise ne se déclenche qu'à la main » | Fermé par **L10**, et l'audit visait juste sur les trois points. (1) La file existe : `queue_governed_template_message` écrit en `STATUS_PENDING` **sans toucher au réseau** — cet état existait sur le modèle depuis l'origine sans être celui d'aucun message réel. (2) L'état est visible : la ligne porte son modèle, son coût et son corps rendu dès la mise en file. (3) La reprise est automatique : `apps/whatsapp/` n'avait **aucun répertoire `management/`** et le registre d'ordonnancement comptait dix-neuf commandes, aucune WhatsApp — d'où la commande `run_whatsapp_queue`, déclarée à cadence horaire. La garde `test_scheduled_commands_declared` ne pouvait pas signaler ce trou : elle vérifie que toute commande **présente sur disque** est planifiée, et un module qui n'en écrit aucune passe en silence. L'absence ne déclenche rien — même angle mort que les budgets, qui mesurent ce qui existe et jamais ce qui manque. Point de conception qui a demandé le plus d'attention : les garde-fous sont revérifiés **à l'envoi** et pas seulement à la mise en file, sans quoi un consentement révoqué entre les deux serait ignoré — ce que WA-2 (« effective immédiatement ») interdit ; et un refus de gouvernance n'entre pas dans le cycle de reprise, contrairement à une panne réseau, les deux portant désormais des états distincts (`error_message`, champ qui n'existait pas). |
| `P2/WA-5` | 🟡 — « la limite de fréquence par destinataire n'a pas été trouvée » | Fermé par **L10**. Elle n'existait effectivement pas. Les deux plafonds ne protègent pas le même objet : le plafond mensuel protège la **facture** de la société, la limite par destinataire protège **une personne** — et le premier n'implique pas le second, cent messages adressés au même numéro coûtant exactement autant que cent messages adressés à cent numéros. Fenêtre **glissante** de 24 h et non journée civile : une journée civile se réinitialise à minuit, ce qui laisserait une boucle émettre son quota deux fois à quelques minutes d'intervalle. Comptage des **lignes du journal**, jamais d'un compteur en cache : `apps/core/throttling.py` est réutilisable en apparence mais convient à une limite HTTP, où un compteur perdu ne coûte qu'une requête de plus ; ici la ligne perdue serait un message réellement parti vers une personne, hors de toute trace. Réglable depuis `/whatsapp/config/` — un plafond éditable seulement par `/admin/` n'est en pratique jamais réglé, comme le régime fiscal avant L17. |
| `P2/WA-1` (volet garde) | 🟡 — contrôle du consentement acquis, **test de non-contournement absent** | Fermé par **L10**. Le critère ne demande pas seulement que le contrôle existe : il demande « un test qu'aucune voie applicative ne le contourne ». `send_governed_template_message` était le point d'entrée unique par convention et par docstring, rien ne l'imposait — un module futur appelant `get_whatsapp_client()` aurait envoyé sans consentement, sans plafond, sans limite de fréquence et sans journal, sans qu'aucun test ne rougisse. La garde inspecte l'import des deux fonctions de transport, avec une **liste d'exception motivée de trois entrées** (la couche de transport elle-même, le point d'entrée gouverné, et le menu WA-8) et un **auto-test du détecteur** — sans lequel elle serait un théâtre de sécurité : `_violations()` renvoie la liste vide aussi bien quand tout est conforme que quand le détecteur est cassé. |
| `P2/WA-10` (routage) | 🟡 — « un seul `WHATSAPP_PHONE_NUMBER_ID` global, un vrai routage par tenant resterait à construire » | Fermé par **L10** — et il faut d'abord dire ce que le lot précédent avait réellement livré, car il ne l'avait pas dit. Ce n'était **pas** un routage par tenant mais un `WHATSAPP_DEFAULT_TENANT_ID` unique pour tout le déploiement : le webhook n'inspectait jamais le `phone_number_id` de l'entrée Meta, si bien que sur une instance multi-sociétés les messages écrits par les clients de la société B tombaient dans le fil de la société A. Le webhook résout maintenant le tenant **par entrée** depuis `changes[].value.metadata`, une même livraison Meta pouvant porter plusieurs numéros ; le tenant par défaut n'est plus qu'un repli pour les instances mono-société. Deux sociétés ne peuvent pas déclarer le même numéro (contrainte d'unicité conditionnelle en base) — sinon le webhook choisirait en silence à laquelle livrer. Et la réponse porte `routed_by_phone_number_id`, sans quoi un déploiement multi-sociétés mal configuré ressemblerait exactement à un déploiement correct. |
| `P2/WA-4`/`WA-8` | ✅ / ✅ | **Défaut non relevé par l'audit, corrigé par L10.** `_maybe_send_intent_menu` envoyait le menu d'accueil par le client brut **sans créer aucune ligne `WhatsAppMessage`**. Ce message partait réellement au client : invisible du journal des échanges — alors que WA-4 exige « envoi journalisé avec modèle, destinataire, statut, catégorie et **coût imputé** » — invisible de l'écran de conversation, et compté pour zéro au plafond de coût comme à la limite de fréquence. Son exemption WA-8 est légitime et portait sur le **consentement** (une réponse dans la fenêtre de service n'est pas une sollicitation business-initiated) ; elle avait été étendue en pratique à la **traçabilité**, ce que rien ne justifiait. Ne pas exiger un consentement n'est pas une raison de ne pas écrire ce qu'on a envoyé. |
| `P1/SAL-6` | 🟡 — « la continuité est garantie par la base ; le test de concurrence exigé n'a pas été trouvé » | Fermé par **L5 (reliquat)**, et **la preuve citée par cet audit était fausse**. Il donnait `accounting/migrations/0005_*` comme « numérotation par trigger DB » : cette migration est en réalité l'immuabilité fine RG-ACC-2, et aucune des dix migrations à trigger du dépôt ne numérote quoi que ce soit. L'attribution est intégralement en Python (`core/services/sequences.py`, dix-huit lignes). Le point que le test devait exercer et qu'aucune lecture rapide ne voit : `select_for_update()` est combiné à `get_or_create()`, or **un `SELECT … FOR UPDATE` ne peut pas verrouiller une ligne qui n'existe pas encore** — la toute première génération pour un triplet (tenant, code, exercice) n'est donc pas protégée par le verrou mais par le `unique_together` et la reprise sur `IntegrityError`. Deux chemins, deux régimes de protection, un seul nommé dans le code. Les quatre tests les exercent tous les deux, plus les deux dimensions du compteur et l'isolation entre sociétés. Il n'existait **aucun test de concurrence dans tout le dépôt** (zéro `threading`, zéro `ThreadPoolExecutor`) ni aucun test de `sequences.py`. |
| `P1/SAL-7` | ❌ — « aucun autosave » | Fermé par **L5 (reliquat)**. Le constat était exact : `lineItems()` gardait ses lignes en mémoire Alpine et `init()` repartait d'une ligne vierge à chaque chargement ; `offline_queue.js` n'intercepte qu'à la **soumission** et seulement **hors ligne**. Mais le défaut le plus fréquent n'a rien à voir avec le réseau et l'audit ne le relevait pas : `quotation_create` **re-rend le formulaire vide après une erreur de validation** (aucun `value=` dans le gabarit, `lines` réinitialisé) — une date mal saisie suffisait à tout perdre, en ligne, sans la moindre panne. Le brouillon répare les deux. Marqué à la soumission pour qu'un devis enregistré ne revienne pas comme brouillon au « Nouveau devis » suivant, où quelqu'un l'aurait saisi deux fois ; clé distincte par écran, les deux partageant le même composant. |
